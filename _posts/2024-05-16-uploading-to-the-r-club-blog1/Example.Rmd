...
If you do not already use github/git, then please familiarize yourself with the basics. There are a number of GUI desktop applications that make using github very easy. Unfortunately I will, not cover using github in today's tutorial.
...
![](images/Rmarkdown Distill Template-01.png)

Be sure to format your YAML appropriately. An example is below

![](images/YAML for post.png)
...
```{r}
Median_ABV_IBU_State<- Beer_data %>% na.omit() %>%
  group_by(State) %>% 
  summarise(`Median ABV`=median(ABV,na.rm = T), `Median IBU`=median(IBU,na.rm=T))


Median_ABV_IBU_State %>% ggplot(aes(x=reorder(State,`Median ABV`),y=`Median ABV`)) + geom_bar(stat = "identity") +  theme(axis.text.x=element_text(size=rel(0.8), angle=90)) + xlab("State")
```

## Which state has the highest median IBUs?

```{r}
Median_ABV_IBU_State %>% ggplot(aes(x=reorder(State,`Median IBU`),y=`Median IBU`)) + geom_bar(stat = "identity") +  theme(axis.text.x=element_text(size=rel(0.8), angle=90)) + xlab("State")
```
...
![](images/Directory 2.png)

![](images/Directory 3.png)

# Checking Your Distill HTML "index.html"

![](images/Distill HTML.png)

# Final Steps

You need to push to github. The blog pulls from the github repository. We will need to figure out a work flow to merge to the master repository. More to come soon.
